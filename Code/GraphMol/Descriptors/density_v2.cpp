...
static const int FEATURE_MASK[300] = {2203, 1366, 1365, 669, 679, 680, 670, 671, 579, 678, 1159, 1358, 582, 1888, 298, 573, 1887, 1889, 3829, 3803, 1317, 1890, 1833, 651, 1884, 3808, 374, 3783, 570, 3781, 1977, 2152, 3768, 687, 1607, 398, 2194, 1895, 1370, 580, 561, 1940, 1157, 822, 1855, 1848, 1835, 823, 572, 1990, 912, 867, 813, 1886, 1892, 707, 1433, 1903, 564, 599, 581, 1885, 571, 1852, 1193, 2669, 338, 553, 2274, 652, 1371, 2412, 357, 345, 1257, 108, 931, 1369, 1245, 814, 1117, 805, 1574, 2303, 411, 3766, 410, 1891, 1811, 724, 1988, 325, 394, 682, 653, 1812, 1984, 1849, 2190, 3767, 546, 543, 1894, 3845, 511, 945, 3776, 1853, 514, 1271, 1156, 448, 2521, 608, 45, 786, 545, 1377, 1819, 1180, 922, 1966, 645, 2150, 1978, 2234, 3034, 662, 1953, 535, 212, 381, 787, 921, 1161, 1109, 1893, 741, 688, 392, 340, 499, 1924, 1169, 420, 1305, 3616, 399, 1957, 161, 634, 1534, 583, 1062, 1448, 672, 1617, 563, 387, 697, 2153, 1631, 2007, 1943, 1921, 1838, 777, 366, 2189, 1372, 1959, 2058, 1843, 1029, 2198, 1948, 314, 138, 1925, 1005, 1328, 1717, 2271, 1983, 389, 904, 832, 1334, 3868, 2837, 1255, 1981, 1175, 297, 962, 2126, 769, 3833, 903, 1860, 309, 1206, 1976, 940, 859, 700, 626, 3811, 544, 412, 1514, 3770, 388, 2168, 2257, 294, 1594, 361, 1492, 751, 226, 655, 796, 684, 371, 719, 1101, 1095, 4092, 939, 1093, 3812, 183, 1368, 886, 750, 3785, 1616, 1407, 858, 617, 230, 1958, 315, 98, 1349, 885, 1367, 218, 1315, 804, 1818, 1152, 4033, 421, 3780, 992, 2502, 452, 1670, 733, 349, 1865, 598, 1337, 1506, 41, 303, 3765, 709, 1854, 965, 1037, 807, 2453, 716, 698, 957, 989, 1553, 1698, 4087, 642, 352, 35, 1232, 3769, 705, 327, 1942, 3876, 1352, 778, 1836, 100, 346, 811, 1993, 949, 1494};
...
DensityV2Predictor::DensityV2Predictor() {
...
bool DensityV2Predictor::isMoleculeTooLarge(const RDKit::ROMol* mol) {
    if (!mol) {
        return true;
    }
...
double DensityV2Predictor::predict(const RDKit::ROMol* mol, int numThreads) const {
...
void DensityV2Predictor::predictBatch(
...
double DensityV2Predictor::predictFromSMILES(const std::string& smiles, int numThreads) const {
...
void DensityV2Predictor::predictFromSMILESBatch(
...
void DensityV2Predictor::extractSMARTSFeatures(
...
void DensityV2Predictor::extractRDKitFeatures(
    const RDKit::ROMol* mol,
    std::vector<double>& features
) const {
    if (!mol) return;
...
void DensityV2Predictor::extractOsmordredFeatures(
    const RDKit::ROMol* mol,
    std::vector<double>& features
) const {
    if (!mol) return;
...
void DensityV2Predictor::extractUnifiedFeatures(
    const RDKit::ROMol* mol,
    std::vector<double>& features
) const {
    if (!mol) {
...
void DensityV2Predictor::preprocessFeatures(std::vector<double>& features) const {
    // Convert inf to nan
...
void DensityV2Predictor::selectFeatures(
