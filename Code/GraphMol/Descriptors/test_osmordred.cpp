...
// ============================================================
// Osmordred v2.0 - NCI 1000 Molecule Golden Reference Test
// Validates descriptor computation against known-good values
// using 1e-5 precision tolerance
// ============================================================

TEST_CASE("Osmordred v2.0 - NCI 1000 Golden Reference Test") {
  const char* rdbase_env = std::getenv("RDBASE");
  std::string rdbase = rdbase_env ? rdbase_env : "";
  ...
  SECTION("Validate NCI 100 molecule golden reference") {
    ...
    // Collect first 1000 valid SMILES and their descriptors
    std::vector<std::string> smiles_list;
    std::vector<std::vector<double>> results_list;
    ...
    if (!golden_file.is_open()) {
      // Generate golden reference (first run)
      INFO("Golden reference file not found. Generating: " << golden_path);
      
      std::ofstream out_file(golden_path);
      if (!out_file.is_open()) {
        WARN("Could not create golden reference file: " << golden_path);
        return;
      }
      ...
      for (size_t i = 0; i < smiles_list.size(); ++i) {
        out_file << smiles_list[i];
        for (const auto& val : results_list[i]) {
          out_file << "," << val;
        }
        out_file << "\n";
      }
      ...
      INFO("Golden reference generated with " << smiles_list.size() << " molecules");
      ...
      INFO("Validating against golden reference: " << golden_path);
      ...
      while (std::getline(golden_file, line) && mol_index < (int)results_list.size()) {
        std::istringstream iss(line);
        std::string golden_smiles;
        std::getline(iss, golden_smiles, ',');
        
        // Verify SMILES matches
        if (golden_smiles != smiles_list[mol_index]) {
          INFO("SMILES mismatch at index " << mol_index);
          mol_index++;
          continue;
        }
        ...
        for (size_t desc_idx = 0; desc_idx < 3585; ++desc_idx) {
          ...
          double current_val = current_result[desc_idx];
          
          // Compare with tolerance
          bool both_nan = std::isnan(golden_val) && std::isnan(current_val);
          bool both_inf = std::isinf(golden_val) && std::isinf(current_val) 
                          && (golden_val > 0) == (current_val > 0);
          
          if (both_nan || both_inf) {
            // NaN == NaN and Inf == Inf for our purposes
            continue;
          }
          
          if (std::isnan(golden_val) != std::isnan(current_val)) {
            INFO("NaN mismatch at molecule " << mol_index << " descriptor " << desc_idx);
            INFO("  Golden: " << golden_val << ", Current: " << current_val);
            mismatches++;
            continue;
          }
          
          // Check relative/absolute difference
          double diff = std::abs(golden_val - current_val);
          double rel_diff = (golden_val != 0) ? diff / std::abs(golden_val) : diff;
          
          if (diff > TOLERANCE && rel_diff > TOLERANCE) {
            INFO("Value mismatch at molecule " << mol_index << " descriptor " << desc_idx);
            INFO("  SMILES: " << golden_smiles);
            INFO("  Golden: " << std::setprecision(15) << golden_val);
            INFO("  Current: " << std::setprecision(15) << current_val);
      ...
      INFO("Validated " << validated << " molecules against golden reference");
